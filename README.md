...
Transaction isolation demo — Session A: Starts a transaction and updates student 101's CGPA to 8.88, but never commits it.
Transaction isolation demo — Session B: Runs SELECT * FROM student concurrently; whether it sees the new 8.88 or the old 8.33 depends on the isolation level — MySQL's default (repeatable read) keeps uncommitted changes hidden from other sessions.
